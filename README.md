...
3.  Double-click  ./QectorWorkbench-1.0.1-x86_64.AppImage  — no installation required
...
| Artifact | Contents |
|:---------|:---------|
| **`QectorWorkbench-1.0.1-x86_64.AppImage`** | `./QectorWorkbench-1.0.1-x86_64.AppImage` + the `qector_decoder_v3` 1.0.0 wheel + `manuals/` + `EULA.txt` + `CITATION.cff` + `RELEASE_MANIFEST.txt` + `SHA256SUMS.txt` + `AIR_GAPPED_HARDENING_STATUS.md` |
| **`./QectorWorkbench-1.0.1-x86_64.AppImage`** | The Windows executable on its own, if you do not want the bundle |
...
`~/.local/share/QectorWorkbench`. Override the location entirely with the
...
|:----------|:------------|
| **OS** | Linux x86_64 (glibc ≥ 2.30), Linux (glibc ≥ 2.30), macOS 12+ |
