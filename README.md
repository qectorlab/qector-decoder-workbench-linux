# QECTOR Decoder Workbench v1.0.0 - macOS DMG

[![Release](https://img.shields.io/github/v/release/qectorlab/qector-decoder-workbench-macos?label=Latest%20Release&style=flat-square)](https://github.com/qectorlab/qector-decoder-workbench-macos/releases/latest)
[![Platform](https://img.shields.io/badge/Platform-macOS-lightgrey?style=flat-square)](#install)

macOS build running the `qector-decoder-v3 1.0.0` backend. This is a single-executable distribution requiring no installer, no admin rights, and no internet connection to run.
...
2. Double-click **`QectorWorkbench.dmg`** to launch the graphical interface.

**Headless MCP Server**
To run the 82-tool `stdio` MCP server (no display needed), launch via command prompt:
```cmd
QectorWorkbench.dmg --mcp
...
* **`QectorWorkbench.dmg`**: The application executable; bundles its own Python runtime, scientific stack, and the `qector-decoder-v3==1.0.0` wheel (activated fully offline on first launch).
...
```text
8e1c7fea5dfb7f0bcb6ccfd2ec42f5866958a7a1f8405cb70a5be4c1df39fb *QectorWorkbench.dmg
...
```powershell
Get-FileHash -Algorithm SHA256 .\QectorWorkbench.dmg
...
*Full documentation is included inside the `manuals/` directory within the `.zip` release. Visit the main repository at [qectorlab/qector-decoder-workbench-macos](https://www.google.com/search?q=https://github.com/qectorlab/qector-decoder-workbench-macos/) for more information.*
