...
Linux build running the `qector-decoder-v3 1.0.1` backend. This is a single-executable distribution requiring no installer, no admin rights, and no internet connection to run.
...
* **`QectorWorkbench.AppImage`**: The application executable; bundles its own Python runtime, scientific stack, and the `qector-decoder-v3==1.0.1` wheel (activated fully offline on first launch).
